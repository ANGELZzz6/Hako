...
import type { MPItem, MPPayer, PSEData } from '../services/paymentService';
import cartService from '../services/cartService';
import './CheckoutPage.css';
import { useAuth } from '../contexts/AuthContext';
import { useCart } from '../contexts/CartContext';
import PSEPaymentForm from '../components/PSEPaymentForm';
...
const CheckoutPage = () => {
  ...
  const handlePSEPayment = async (pseData: PSEData) => {
    setLoading(true);
    setError(null);
    try {
      console.log('Procesando pago PSE con datos:', pseData);
      
      const paymentResult = await paymentService.processPSEPayment(pseData, items, payer);
      
      console.log('Resultado del pago PSE:', paymentResult);
      
      // Si el pago PSE está pendiente, redirigir al banco
      if (paymentResult.status === 'pending' && paymentResult.external_resource_url) {
        console.log('Pago PSE pendiente, redirigiendo al banco:', paymentResult.external_resource_url);
        // Eliminar productos del carrito ya que el usuario se compromete a pagar
        await removePurchasedItems(items);
        // Redirigir al banco
        window.location.href = paymentResult.external_resource_url;
        return;
      }
      
      // Si el pago fue exitoso, eliminar los productos del carrito
      if (paymentResult.status === 'approved') {
        console.log('Pago PSE aprobado, eliminando productos del carrito...');
        await removePurchasedItems(items);
      }
      ...
      console.error('Error procesando el pago PSE:', err);
      ...
      if (err.status && err.status_detail) {
        ...
        setError('Error procesando el pago PSE. Por favor, intenta nuevamente.');
      }
    } finally {
      setLoading(false);
    }
