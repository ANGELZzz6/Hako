...
interface Product {
  ...
  precio: number;
  imagen_url: string;
  adminRating?: number; // Nueva propiedad para calificación
}

interface ProductCardProps {
  producto: Product;
  onClick?: (id: string) => void;
  showStars?: boolean; // Nueva propiedad para mostrar estrellas opcionalmente
}

const ProductCard: React.FC<ProductCardProps> = ({ producto, onClick, showStars }) => {
  ...
  const renderStars = () => {
    if (!showStars || !producto.adminRating || producto.adminRating <= 0) return null;

    const rating = Math.min(5, Math.max(0, producto.adminRating));
    const fullStars = Math.floor(rating);
    const hasHalfStar = rating % 1 >= 0.5;

    return (
      <div className="estrellas-container mb-2" title={`Calificación: ${rating}/5`}>
        {[...Array(5)].map((_, i) => {
          if (i < fullStars) {
            ...
            return <i key={i} className="bi bi-star text-muted op-5"></i>;
          }
        })}
